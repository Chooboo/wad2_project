...
class Registration_and_Login_Tests(TestCase):
    ...
    def test_new_registration_view_exists(self):
        ...
        self.assertEqual(url, '/accounts/register/', f"{FAILURE_HEADER}Have you created the musicquiz:register URL mapping correctly? It should point to the new register() view{FAILURE_FOOTER}")
    ...
    #def test_registration_template(self):
        ...
        #self.assertTrue(os.path.exists(template_path), f"{FAILURE_HEADER}We couldn't find the 'registration_register.html' template in the 'templates/musicquiz/' directory. Did you put it in the right place?{FAILURE_FOOTER}")
        ...
        #self.assertTrue(re.search(full_title_pattern, content), f"{FAILURE_HEADER}The <title> of the response for 'registration_register' is not correct. Check your register.html template, and try again.{FAILURE_FOOTER}")
        #self.assertTrue(re.search(block_title_pattern, template_str), f"{FAILURE_HEADER}Is registration_register.html using template inheritance? Is your <title> block correct?{FAILURE_FOOTER}")
